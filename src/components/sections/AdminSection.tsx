...
import { PERMS, PRESETS, type PermissionId, type HHEvent, type OrgService, type OrgTestimonial, type OrgQuickLink } from "@/lib/types";
...
function OrgProfilePanel({ flash }: { flash: (m: string) => void }) {
  const { org } = useOrg();
  ...
  async function save() {
    ...
    try {
      ...
      setLogoFile(null);
      flash("Organization profile saved — your public page is updated");
    } catch (err) {
  ...
  return (
    <Blueprint style={{ gridColumn: "1/-1" }}>
      ...
      <p className="text-muted" style={{ fontSize: 11, margin: "0 0 13px" }}>
        Your name, logo and slogan appear across every coordination page. Everything below feeds your public page at{" "}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,260px),1fr))", gap: 12 }}>
        ...
        <label>
          ...
          <Input value={slogan} onChange={(e) => setSlogan(e.target.value)} placeholder="A short line under your name" />
        </label>
        <div>
      ...
      <div style={{ marginTop: 12 }}>
        ...
        <textarea className="input" value={about} onChange={(e) => setAbout(e.target.value)} placeholder="A few sentences about your organization" style={{ width: "100%", minHeight: 80 }} />
      </div>
